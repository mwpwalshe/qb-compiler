...
correctness (**ObservableGate**). All of it runs in CI, forever, for free.
...
preflight bundle, Result Passport). Prove value for free, charge only once teams depend on the receipts.
...
ObservableGate is the first of them (fidelity receipt · drift receipt · ObservableGate receipt ·
SafetyGate receipt · QEC preflight receipt · Result Passport). The Pro layer is additive and never
blocks the open-source path.
