...
def parse_iqm_calibration(data: dict) -> BackendProperties:
    """Parse a QubitBoost ``calibration_hub/braket/iqm_garnet*.json`` dict
    into :class:`BackendProperties`.

    IQM backends use CZ as the native two-qubit gate and PRX for
    single-qubit rotations.

    Parameters
    ----------
    data:
        Parsed JSON from a QubitBoost IQM calibration file.
    ...
    return BackendProperties(
        backend=data.get("backend_name", "unknown_iqm"),
        provider="iqm",
...
def load_iqm_calibration(path: str | Path) -> BackendProperties:
    """Convenience: load JSON file and parse in one call."""
    with open(path) as fh:
        data = json.load(fh)
    return parse_iqm_calibration(data)
