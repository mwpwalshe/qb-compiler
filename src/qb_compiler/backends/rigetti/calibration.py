...
def parse_rigetti_calibration(data: dict) -> BackendProperties:
    """Parse a QubitBoost ``calibration_hub/braket/rigetti_ankaa*.json`` dict
    into :class:`BackendProperties`.

    The Rigetti calibration data uses the same QubitBoost JSON schema as IBM,
    with vendor-specific differences in typical gate names (CZ-based) and
    error magnitudes.

    Parameters
    ----------
    data:
        Parsed JSON from a QubitBoost Rigetti calibration file.
    ...
    return BackendProperties(
        backend=data.get("backend_name", "unknown_rigetti"),
        provider="rigetti",
...
def load_rigetti_calibration(path: str | Path) -> BackendProperties:
    """Convenience: load JSON file and parse in one call."""
    with open(path) as fh:
        data = json.load(fh)
    return parse_rigetti_calibration(data)
